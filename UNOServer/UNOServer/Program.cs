...
namespace UNOServer
{
    ...
    class Program
    {
        ...
        public static void AnalyzingMessage(string receivedata, PLAYER User)
        {
            ...
            switch (Message[0]) //Xét phần tử đầu tiên trong mảng Message chứa loại thông điệp (phần đầu tiên trong thông điệp) được gửi từ client
            {
                ...
                case "Getpassword":
                    ...
                    break;
                case "BuyCard":
                    UpdateTienInDatabase(Message[1],int.Parse(Message[2]));
                    break;
                case "Joingame":
         ...
         * Login;ID;<Password>                                                        | LoginSuccessful/LoginFail;ID/<Thông báo lỗi nếu lỗi>;Tien (nếu thành công)
         ...
         * Disconnect;ID                                                              | Result;ID;Diem;Rank;Tien
         *                                                                            | NotEnoughPlayers;ID
         * LƯU Ý: 
        ...
        */
        ...
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\ledan\source\repos\NEWUNOOFFICIAL-main\UNOServer\UNOServer\UserDatabase.mdf;Integrated Security=True;";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    {
                        //Tìm socket người chơi tương ứng trong PLAYERLIST0 và gán id cho player đó
                        foreach (var player in PLAYERLIST0)
                        {
                            if (player.PlayerSocket == User.PlayerSocket)
                            {
                                player.ID = Message[1];
                                break;
                            }
                        }
                        //Chọn ra người chơi trong PLAYERLIST0 có ID trùng với ID của người chơi đăng nhập và gọi hàm UpdatePlayerTienFromDatabase với tham số là người chơi
                        foreach (var player in PLAYERLIST0)
                        {
                            if (player.ID == Message[1])
                            {
                                UpdatePlayerTienFromDatabase(player);
                                break;
                            }
                        }
                        //Gửi LoginSuccessful, ID của người chơi và số tiền của người chơi
                        string SendData = "LoginSuccessful;" + Message[1] + ";" + User.Tien;
                        byte[] data = Encoding.UTF8.GetBytes(Encrypt(SendData));
                        User.PlayerSocket.Send(data);
        ...
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\ledan\source\repos\NEWUNOOFFICIAL-main\UNOServer\UNOServer\Database1.mdf;Integrated Security=True;";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
        ...
        private static void HandleDiem(string[] Message, PLAYER User)
        {
            for (int i = 0; i < PLAYERLIST.Count; i++)
            {
                if (PLAYERLIST[i].ID == WinnerName)
                {
                    ...
                    UpdateTienInDatabase(PLAYERLIST[i].ID, PLAYERLIST[i].Diem); //Cập nhật điểm vào database
                    Console.WriteLine("Điểm của người thắng " + WinnerName + ": " + PLAYERLIST[i].Diem);
                    break;
                }
            }
            UpdatePlayerTienFromDatabase(User);
        }
        private static void UpdateTienInDatabase(string playerId, int newTien)
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\ledan\source\repos\NEWUNOOFFICIAL-main\UNOServer\UNOServer\UserDatabase.mdf;Integrated Security=True"; // Chuỗi kết nối đến SQL Server
            string query = "UPDATE TaiKhoan SET Tien = Tien + @newTien WHERE TenTaiKhoan = @playerId";

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@newTien", newTien);
                ...
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        Console.WriteLine("Player's Tien updated successfully.");
                    }
                    else
                    {
                        Console.WriteLine("No player found with the given TenTaiKhoan.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("An error occurred while updating the player's Tien: " + ex.Message);
                }
            }
        }
        private static void UpdatePlayerTienFromDatabase(PLAYER player)
        {
            string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\ledan\source\repos\NEWUNOOFFICIAL-main\UNOServer\UNOServer\UserDatabase.mdf;Integrated Security=True"; // Chuỗi kết nối đến SQL Server
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string query = "SELECT Tien FROM TaiKhoan WHERE TenTaiKhoan = @TenTaiKhoan";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@TenTaiKhoan", player.ID);

                    connection.Open();
                    object result = command.ExecuteScalar();
                    if (result != null)
                    {
                        player.Tien = Convert.ToInt32(result);
                    }
                }
            }
        }
        ...
        private static void HandleAfterMatch(string[] Message, PLAYER User)
        {
            ...
            if ((DemRestart + DemFinish) == PLAYERLIST.Count)
            {
                if (DemFinish > 0) //Có người chọn Finish thì gưi thông điệp Result: hiện màn hình xếp hạng kết thúc và điểm số.
                {
                    ...
                    foreach (var user in PLAYERLIST)
                    {
                        string SendData = "Result;" + user.ID + ";" + user.Diem + ";" + user.Rank + ";" + user.Tien;
                        byte[] data = Encoding.UTF8.GetBytes(Encrypt(SendData));
                        user.PlayerSocket.Send(data);
                        Thread.Sleep(200);
                    }
